...
void solve(){
    int n;
    cin >> n;
    V<V<array<mint, 3>>> dp(n + 2, V<array<mint, 3>>(n + 2, {-1, -1, -1}));
    auto f = [&] (int i, int j, int k, auto&& self) -> mint {
        if(i > n) return j == 1;
        if(dp[i][j][k] != -1) return dp[i][j][k];
        mint ans = (j + 1) * self(i + 1, j + 1, 2, self);
        int gaps = 2 * j - k;
        ans += gaps * self(i + 1, j, 1, self);
        gaps = j - 1 - k;
        if(j > 1 && gaps > 0) ans += gaps * self(i + 1, j - 1, 0, self);
        return dp[i][j][k] = ans;
    };
    mint ans = f(2, 1, 2, f);
    out(ans);
