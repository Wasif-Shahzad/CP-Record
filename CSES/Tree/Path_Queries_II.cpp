...
struct Node {
    ...
    void apply(const Tag& t) { if(t.v != -1) v = t.v; }
};
...
template<bool VALS_IN_EDGES> struct HLD {
    ...
    void build(int R = 0) {
        ...
        for(int i=0; i<N; ++i) initialNodes[i] = Node(0); 
    ...
    int queryPath(int u, int v) {
        int res = 0;
        processPath(u, v, [&](int l, int r) { 
            res = max(res, tree.query(l, r + 1).v); 
        });
        return res;
    }
    ...
    int querySubtree(int x) {
        return tree.query(pos[x] + VALS_IN_EDGES, pos[x] + sz[x]).v;
    }
};
