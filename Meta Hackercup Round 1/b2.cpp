//
// b2.cpp
// Created by Wasif on 10/18/25 at 23:15:36.
//
...
struct Factorizer {
    ...
    vector<pair<int, int>> factorize(int64_t n, bool check_small = true) {
...
Factorizer factorizer(1e6, 1e6);
const int mod = 1e9+7;
int fact[10000005], ifact[10000005];
...
void solve() {
    ...
    for(int i = 1; i * i <= b; i++) {
        if(b % i == 0) {
            if(i <= a) {
                auto facts = factorizer.factorize(i);
                int comb1 = 1;
                for(auto [p, c]: facts) {
                    comb1 = (comb1 * ncr(c + n - 1, n - 1)) % mod;
                }
                auto other = factorizer.factorize(b / i);
                int comb2 = 1;
                for(auto [p, c]: other) {
                    comb2 = (comb2 * ncr(c + n - 1, n - 1)) % mod;
                }
                ans += (comb1 * comb2) % mod;
                if(b / i <= a) ans += (comb1 * comb2) % mod;
            }
        }
    }
    cout << ans << '\n';
