...
echo ":wrench: Compiling your spaghetti (and checking for Python snake oil) :spaghetti:"

# Only compile Gen and Brute now (since Sol is Python)
...
if [ -f "Sol.py" ]; then
    echo ":snake: Found Sol.py. Interpreted languages... how slow."
else
    echo ":x: Sol.py not found! Are you trying to trick me?"
    exit 1
fi
...
declare -a ROASTS=(
    ...
    "Python? Really? Fine, if it passes..."
)
...
for ((i=1; i<=MAX_TESTS; i++)); do
    ...
    # CHANGED: Use python3 for the solution
    ...
    if ! diff -q output.txt expected.txt > /dev/null; then
        ...
        cat input.txt
        echo -e "${YELLOW}--- YOUR OUTPUT (Python) ---${RESET}"
        cat output.txt
