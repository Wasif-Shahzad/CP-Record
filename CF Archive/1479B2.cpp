//
// 1479B2.cpp
// Created by wasifshahzad on 05/25/26 at 07:24:03.
//
...
struct Node {
    ...
    Node(int x = 1e7, int ll = 0) : v(x), len(ll) {} 
    Node operator+(const Node &other) {
        return Node(min(v, other.v), len + other.len);
    }
    void apply(const Tag& t) { if(v != (int)1e7) v += t.v; }
};
...
void solve() {
    ...
    vector<Node> t1(n + 1, Node(n + 5, 1));
    t1[0].v = 1;
    LazySeg dp(t1);
    for(int i = 2; i <= n; i++) {
        int left = dp.query(0, a[i]).v, right = dp.query(a[i] + 1, n + 1).v;
        int other = min(min(left, right) + 1, dp.query(a[i], a[i] + 1).v);
        dp.update(0, n + 1, Tag(a[i] != a[i - 1]));
        dp.update(a[i - 1], Node(other, 1));
    }
    cout << dp.query(0, n + 1).v << '\n';
}
