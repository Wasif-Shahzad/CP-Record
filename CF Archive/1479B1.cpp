//
// 1479B1.cpp
// Created by wasifshahzad on 05/24/26 at 16:59:26.
//
...
struct Node {
    ...
    Node operator+(const Node &other) {
        return Node(max(v, other.v), len + other.len);
    }
...
void solve() {
    ...
    vector<Node> t1(n + 1, Node(0, 1));
    t1[0].v = 1;
    LazySeg dp(t1);
    for(int i = 2; i <= n; i++) {
        int left = dp.query(0, a[i]).v, right = dp.query(a[i] + 1, n + 1).v;
        int other = max(max(left, right) + 1, dp.query(a[i], a[i] + 1).v);
