//
// b.cpp
// Created by wasifshahzad on 01/05/26 at 20:24:44.
//
...
struct Tag {
    ...
    Tag(int x = 0) : v(x) {}
    void apply(const Tag& other) { if(other.v != 0) v ^= 1; }
};
...
struct Node {
    ...
    void apply(const Tag& t) { if(t.v != 0) v = len - v; }
};
...
struct LazySeg {
    ...
    int find(int k, int x, int l, int r) {
        ...
        int m = (r + l) / 2;
        auto left = query(l, m);
        if(left.v >= k) return find(k, 2 * x, l, m);
        return find(k - left.v, 2 * x + 1, m, r);
    }
