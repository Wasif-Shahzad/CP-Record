//
// b.cpp
// Created by wasifshahzad on 01/06/26 at 17:45:58.
//
...
struct Tag {
    ...
    Tag(int x = 0, int y = 0) : a(x), d(y) {}
    void apply(const Tag& other) { a += other.a, d += other.d; }
};
...
struct LazySeg {
    ...
    void push(int v) {
        apply(2 * v, lazy[v]); 
        Tag right_tag = lazy[v];
        right_tag.a += t[2 * v].len * right_tag.d;
        apply(2 * v + 1, right_tag);
        lazy[v] = Tag();
    }
...
signed main() {
    ...
    while(m--) {
        ...
        if(t == 1) {
            ...
            cin >> l >> r >> a >> d;
            l--;
            seggy.update(l, r, Tag(a, d));
        } else {
            int i;
            cin >> i;
            i--;
            auto res = seggy.query(i, i + 1);
            cout << res.v << '\n';
        }
    }
}