//
// d.cpp
// Created by wasifshahzad on 01/06/26 at 21:27:50.
//
...
struct Tag {
    ...
    Tag(int x = 0, int type = -1) : v(x), t(type) {}
    void apply(const Tag& other) {  
        if(other.t == t) v += other.v;
...
struct Node {
    ...
    Node(int x = 0, int A = 0, int B = 0, int ll = 1) : v(x), a(A), b(B), len(ll) {} 
...
signed main() {
    ...
    while(m--) {
        ...
        if(t == 1) {
            ...
            cin >> l >> r >> d;
            l--;
            seggy.update(l, r, Tag(d, 1));
        } else {
            int l, r;
            cin >> l >> r;
            l--;
            seggy.update(l, r, Tag(-l, 2));
            auto res = seggy.query(l, r);
            cout << res.v << '\n';
            seggy.update(l, r, Tag(l, 2));
        }
    }
